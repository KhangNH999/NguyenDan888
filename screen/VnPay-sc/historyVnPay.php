...
if (isset($_GET['vnp_BankCode']) && $_GET['vnp_BankCode'] == "NCB") {
    ...
    $sql_update_lichsu =  mysqli_query($mysqli, "UPDATE lichsudatsan SET hinhthucthanhtoan='VnPay', xacnhandatsan=2 WHERE malichsu='" . $malichsu . "'");
    //Add notify
    ...
    // if ($sql_thank) {
    //     $sql_update = mysqli_query($mysqli, "UPDATE san SET tinhtrangsan = '0' WHERE masan='" . $masan . "'");
    try {
        ...
        $mail->Body    = '<p>Cảm ơn quý khách đã đặt sân trên Hinada</p>
            ...
            <p>Quý khách vui lòng đến đúng giờ. Cảm ơn quý khách đã đọc gmail này.</p>
            ';
